...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    def incrementMatchingPair(pairs: List[(Char, Int)], c: Char): List[(Char, Int)] = pairs match {
      case Nil => (c, 1) :: Nil
      case h :: t => if (isPairMatched(h, c)) (h._1, h._2 + 1) :: t else h :: incrementMatchingPair(t, c)
    }

    def count(chars: List[Char], pairs: List[(Char, Int)]): List[(Char, Int)] = chars match {
      case Nil => pairs
      case h :: t => count(t, incrementMatchingPair(pairs, h))
    }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    def insertRetainingSort(li: List[Leaf], l: Leaf): List[Leaf] = li match {
      case Nil => List(l)
      case h :: t => if (l.weight <= h.weight) l :: li else h :: insertRetainingSort(t, l)
    }

    def buildList(freqs: List[(Char, Int)], accum: List[Leaf]): List[Leaf] = freqs match {
      case Nil => accum
      case h :: t => buildList(t, insertRetainingSort(accum, Leaf(h._1, h._2)))
    }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    ...
    def getMatchingSubTree(tr: CodeTree, bit: Bit): CodeTree = tr match {
      case Fork(l, r, c, w) => if (bit == 0) l else r
      case _ => throw new IllegalArgumentException("Error:Subtree of Leaf")
    }

    def go(tr: CodeTree, bits: List[Bit], accum: List[Char]): List[Char] = bits match {
      case Nil => accum
      case (head :: tail) =>
        getMatchingSubTree(tr, head) match {
          case Fork(l, r, c, w) => go(Fork(l, r, c, w), tail, accum)
          case Leaf(c, w) => go(tree, tail, accum ::: List(c))
        }
    }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case Nil => throw new IllegalArgumentException("codeBits.not.found")
    case (c, b) :: t => if (c == char) b else codeBits(t)(char)
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    ...
    def go(tr: CodeTree, bitsAccum: List[Bit]): CodeTable = tr match {
      case (Leaf(c, w)) => List((c, bitsAccum))
      case (Fork(l, r, c, w)) => mergeCodeTables(go(l, bitsAccum ::: List(0)), go(r, bitsAccum ::: List(1)))
    }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def go(text: List[Char]): List[Bit] = text match {
      case Nil => Nil
      case h :: t => codeBits(codeTable)(h) ::: go(t)
    }
    go(text)
  }
}
