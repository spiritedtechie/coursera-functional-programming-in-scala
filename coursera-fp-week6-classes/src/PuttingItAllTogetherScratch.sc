...
object PuttingItAllTogetherScratch {

  val in = Source.fromFile("/tmp/linuxwords.txt") //> in  : scala.io.BufferedSource = non-empty iterator
  // ("http://lamp.epfl.ch/files/content/sites/lamp/files/teaching/progfun/linuxwords.txt", "utf8")
  ...
  val charCode: Map[Char, Char] = for {
    (n, chars) <- mnem
    c <- chars
  } yield c -> n                                  //> charCode  : Map[Char,Char] = Map(E -> 3, X -> 9, N -> 6, T -> 8, Y -> 9, J -
