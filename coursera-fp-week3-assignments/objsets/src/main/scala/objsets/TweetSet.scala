...
abstract class TweetSet {
  ...
  def mostRetweetedAccum(acc: Tweet): Tweet
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = mostRetweetedAccum(elem)

  def mostRetweetedAccum(acc: Tweet): Tweet = {
    if (elem.retweets > acc.retweets) right.mostRetweetedAccum(left.mostRetweetedAccum(elem))
